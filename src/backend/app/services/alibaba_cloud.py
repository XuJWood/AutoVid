...
class WanxVideoService(BaseAIService):
    ...
    支持模型:
      - wan2.7-t2v / wan2.6-t2v / wan2.5-t2v: 文生视频 2-15s, 720P/1080P, 多镜头叙事, 声画同步
      - wan2.7-i2v / wan2.6-i2v: 图生视频 2-15s, 首帧/首尾帧, 音频驱动口型同步
      - wanx2.1-t2v-turbo/plus: 旧版文生视频 5s (兼容)
      - wanx2.1-i2v-turbo/plus: 旧版图生视频 5s (兼容)
    """
    ...
    def _is_new_gen(self) -> bool:
        """wan2.5/2.6/2.7 series use resolution+ratio+duration format"""
        return any(v in self.model for v in ["wan2.7", "wan2.6", "wan2.5"])

    @property
    def _is_i2v(self) -> bool:
        return "i2v" in self.model
    ...
    async def generate(
        ...
        **kwargs
        ...
        try:
            ...
            # negative_prompt (wan2.7+)
            negative_prompt = kwargs.get("negative_prompt")
            if negative_prompt:
                input_data["negative_prompt"] = negative_prompt

            if self._is_i2v:
                # i2v: media array for first_frame + driving_audio
                media = []
                if image_url:
            ...
            elif self._is_new_gen and audio_url:
                # wan2.7+ t2v: audio_url directly in input
                input_data["audio_url"] = audio_url

            # Build parameters based on model generation
            if self._is_new_gen:
                if self._is_i2v:
                    parameters = {
                ...
                # Legacy wanx2.1: size-based parameters
                if self._is_i2v:
                    parameters = {
                        "resolution": resolution,
                        "duration": duration,
            ...
            if kwargs.get("watermark") is not None:
                parameters["watermark"] = kwargs["watermark"]
            ...
            async with httpx.AsyncClient(timeout=300.0) as client:
                ...
                task_url = f"{self.base_url}/api/v1/tasks/{task_id}"
                max_wait = max(900, duration * 60)  # scale with duration
                waited = 0
                    ...
                    if task_status == "SUCCEEDED":
                        ...
                        return GenerationResult(
                            ...
                            data={
                                ...
                                "actual_prompt": task_result.get("output", {}).get("actual_prompt"),
                            }
                        )
