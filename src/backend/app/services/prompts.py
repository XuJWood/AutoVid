...
## 漫剧核心法则
1. **20秒完整叙事**: 每集 = 开场(3s)→发展(5s)→高潮(7s)→收尾(5s)，是一个独立的故事单元
2. **反转驱动**: 每2-3集一个反转——身份反转、关系反转、认知反转、局势反转
3. **爽点密集**: 被打压→反击→打脸→胜利，循环出现
4. **情绪过山车**: 每集完成一个完整情绪周期：紧张→愤怒→释放→满足
5. **角色鲜明**: 好人要有软肋，坏人要有动机，每个角色都有记忆点
6. **对话精彩**: 台词有信息量、有冲突感、有金句潜质，不说废话
...
## 集数规划
- 短篇: 8-20集，快节奏推进，每3集一个大转折
- 中篇: 20-50集，有支线剧情，每5-8集一个大转折
- 长篇: 80-120集，多线叙事，有完整的起承转合
...
4. 每集结尾必须有钩子
5. 角色外观描述具体，符合日系动漫风格
6. environment/location/mood 必须写详细——这直接影响AI视频生成质量
7. 对话 speaker 必须用准确的角色名，emotion 必须标注
8. 场景描述要包含光线、色调、氛围，越具体越好"""
...
{{
  ...
  "episodes": [
    ...
    ...继续生成直到达到要求的集数...
      ...
      "twist": "本集反转点（如有，否则留空）",
      "description": "本集20秒故事概述（发生了什么，情绪如何变化，结局是什么）",
      "script": "本集完整剧本——包含场景描述、每个镜头画面、所有角色对话和动作指示",
      "dialogues": [
      ...
      "shots": [
        {{
          "id": 1,
          "type": "特写/近景/中景/全景/远景",
          "angle": "平视/俯拍/仰拍/侧拍/过肩",
          "movement": "固定/推/拉/摇/移/跟/急摇",
          "description": "日系动漫画面详细描述——角色+动作+背景+光线+色调+氛围效果（如樱花飘落/速度线/集中线/光晕），80字以上",
          "action": "角色具体动作（越具体越好）",
          "dialogue": "角色名：台词内容",
          "narration": "旁白内容（如无则留空）",
          "emotion": "本镜头情绪",
          "duration": 3-8,
          "transition": "硬切/淡入淡出/叠化/闪白"
        }}
      ]
    }}
  ]
...
- 台词格式 "角色名：内容"，dialogue 数组中 speaker 和 text 分开
- 每集 dialogue 数组至少4个元素
- 角色的 appearance 和 clothing 必须写满字数，这是AI生图的依据
- environment 必须详细——光线、色调、氛围、背景——这是AI生视频的依据
- shot description 必须具体——画面+动作+光线+氛围+动漫特效
...
- Version: 4.0
- Language: 中文
- Description: 顶级漫剧导演，精通日系动漫镜头语言和AI视频生成。每集20秒，能精准地将剧本转化为视觉化的分镜脚本。
...
2. **运镜**: 推=聚焦表情 / 拉=揭示全貌 / 跟=随角色移动 / 静态=日系静止帧美学
...
STORYBOARD_USER_PROMPT = """请为以下漫剧内容生成详细的剧集视频提示词：
...
## 输出要求
请以JSON格式输出本集的视频生成提示词——描述越详细，AI生成的视频质量越高：
...
{{
  ...
  "video_prompt": "用于AI生成20秒动漫视频的中文提示词（角色外观+动作描述+场景氛围+光线色调+日系动漫风+流畅动画+高画质+特效元素，200字以内）",
  "environment_detail": {{
    "location": "具体地点名",
  ...
  "key_frames": [
    {{"time": "0-5s", "description": "开场画面——角色+动作+背景+光线+氛围"}},
    {{"time": "5-12s", "description": "发展画面——冲突/对话+运镜+情绪"}},
    {{"time": "12-20s", "description": "高潮收尾——关键动作/表情+反转/钩子+收束"}}
  ]
}}
```

## 视频提示词公式（全部使用中文）
动漫生视频 = 日系动漫风 + 角色外观 + 动作描述 + 场景氛围 + 光线色调 + 精致的色彩 + 流畅的动画 + 高画质 + 二次元"""
